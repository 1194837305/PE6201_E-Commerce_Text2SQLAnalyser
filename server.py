...
Resolve multilingual, abbreviated, misspelled or loose user terms semantically against ACTUAL values (e.g. 日本 means stored country Japan). Never require stored English spelling and never invent absent values. Prefer normalized tables and documented joins for cross-domain questions. Revenue=SUM(order_items.total_sales); Profit=SUM(order_items.profit); DAU=count distinct events.customer_id per event_date; MAU=count distinct events.customer_id per month. Dates are YYYY-MM-DD. Limit grouped output to 50.
